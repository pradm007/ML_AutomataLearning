...
from architect.automaton.state import State
from architect.automaton.fsa import FSA
...
class DFA(FSA):
    ...
    def minimal_diverging_suffix(self, state1, state2): #gets series of letters showing the two states are different,
        ...
        while len(new_states) > 0:
            ...
            s1,s2 = state_pair
            if len([q for q in [s1,s2] if q in self.accept_states])== 1: # intersection of self.F and [s1,s2] is exactly one state,
            ...
            for a in self.alphabet:
                ...
                next_state_pair = (self.transition(s1, a),self.transition(s2, a))
