from architect import utils, automaton, algorithms
from architect.oracle.oracle import Oracle
from neuralnets import Teacher
from typing import Set
from observationtable.ObservationTable import TableTimedOut
...
class Learner:
    ...
    def __init__(self, alphabet: Set[str],
        ...
        self._learners = {
            'gold'  : lambda: algorithms.Gold(pos_examples, neg_examples, self._alphabet, self._teacher),
            'rpni'  : lambda: algorithms.RPNI(pos_examples, neg_examples, self._alphabet, self._teacher),
            'lstar' : lambda: algorithms.LSTAR(self._alphabet, oracle, self._teacher, time_limit, start),
            'nlstar': lambda: algorithms.NLSTAR(self._alphabet, oracle, self._teacher, time_limit, start)
        }
    ...
    def learn_grammar(self) -> automaton.DFA:
        ...
        :rtype: Automaton
        """
        inferrAlgo = self._learners[self._algorithm]()
        try:
            inferrAlgo.learn()
        ...
        # fsa = self._learners[self._algorithm]._teacher[-1]
        fsa = inferrAlgo._teacher.dfas[-1]
