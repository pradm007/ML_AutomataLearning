from architect.algorithms.active.active_learner import ActiveLearner
from architect.oracle.oracle import Oracle
from architect.oracle.active_oracle import ActiveOracle
from architect.algorithms.active.nlstar.observation_table import ObservationTable
from architect.algorithms.active.nlstar.row import Row
from architect.automaton import State
from architect.automaton.nfa import NFA
...
class NLSTAR(ActiveLearner):
    ...
    def __init__(self, alphabet: Set[str], oracle: Oracle, teacher: Teacher,
                 time_limit: int=600,
                 start: int=0):
        """
        ...
        :type oracle: Oracle
        """
        super().__init__(alphabet, oracle, teacher)
    ...
    def _assert_not_timed_out(self):
        if not None is self._time_limit:
            if clock() - self._start > self._time_limit:
